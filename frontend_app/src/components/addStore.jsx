...
class AddStoreForm extends Component {
	...
	async componentDidMount() {
		...
		try {
			...
			this.setState({ token, store: { user: token._id } });
		} catch (ex) {
	...
	handleSubmit = async (e) => {
		...
		try {
			const response = await http.post(`${config.apiEndpoint}/api/store`, data, {
				headers: { 'x-auth-token': localStorage.getItem('token') }
			});
			if (response.status === 201) {
				toast.success('Store created successfully!');
				this.setState({ data: response.data, store: { name: '', address: '' } });
			}
		...
		return (
			<div>
				<DashBoardDrawer user={this.state.token}>
					<ToastContainer />
					<h2 className="h2 text-white text-center mb-5">Add Your Store</h2>
