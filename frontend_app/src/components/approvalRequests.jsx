...
class PendingStores extends Component {
	state = {
		stores: [],
		status: true
	};
	...
	async componentDidMount() {
		...
		try {
			...
			this.setState({ stores: data.filter((res) => res.isApproved === false), token });
		} catch (ex) {
	...
	handleApproved = async (storeId) => {
		try {
			const response = await http.put(`${config.apiEndpoint}/api/store/approved/${storeId}`, {
				isApproved: true
			});
			if (response.status === 200) {
				const stores = this.state.stores.filter((store) => store._id !== storeId);
